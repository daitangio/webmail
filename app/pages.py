...
def home():
    ...
    if bool(msg_num) != True:
        ...
        if id_list != []:
            ...
            top_msg_on_page = (
                int(id_list[-1]) - (num_msg_per_page * page_num) + num_msg_per_page
            )
            full_url = url_for(
                ".home", msg_num=top_msg_on_page, folder=folder, page_num=page_num
            )
            imap.close()
            imap.logout()
            return redirect(full_url)
    ...
    if get_id_list(imap, folder) == []:
        ...
        if int(page_num) != correct_page_num:
            full_url = url_for(
                ".home", page_num=correct_page_num, folder=folder, msg_num=msg_num
            )
            imap.close()
            imap.logout()
            return redirect(full_url)
        ...
        # print("-----------")
        num_pages = int(last_msg_num / num_msg_per_page) + (
            last_msg_num % num_msg_per_page > 0
        )
        ...
        messages = message_list(
            ...
            folder,
            range(
                (last_msg_num - (num_msg_per_page * page_num) + 1),
                (last_msg_num - ((page_num - 1) * num_msg_per_page) + 1),
            ),
        )
    ...
    return render_template(
        ...
        body=body,
        num_pages=num_pages,
        num_messages_in_folder=num_messages_in_folder,
    )
...
def send():
    ...
    if request.method == "POST":
        ...
        return render_template(
            "send.html", user=current_user, body=body, to_addr=to_addr, subject=subject
        )
...
def settings():
    ...
    if request.method == "POST":
        ...
        if page == "general":
            if bool(num_msg_per_page) != True or bool(del_button_behavior) != True:
                ...
                return render_template(
                    "settings.html", user=current_user, page="general"
                )
            else:
            ...
            if (
                bool(outgoing_hostname) != True
                or bool(incoming_hostname) != True
                or bool(smtp_port) != True
                or bool(imap_port) != True
            ):
    ...
    return render_template(
        ...
        user=current_user,
        page="general",
        num_msg_per_page=num_msg_per_page,
        del_button_behavior=del_button_behavior,
        incoming_hostname=incoming_hostname,
        outgoing_hostname=outgoing_hostname,
        smtp_port=smtp_port,
        imap_port=imap_port,
    )
...
def trash():
    ...
    response = move_msg_to_trash(
        imap, msg_num, folder, del_pref=gen_settings.del_button_behavior
    )
